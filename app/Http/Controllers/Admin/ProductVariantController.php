...
class ProductVariantController extends Controller
{
    ...
    public function storeMany(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'variants.*.price' => 'required|numeric|min:0',
            'variants.*.stock' => 'required|integer|min:0',
            'variants.*.image' => 'required|file|image|mimes:jpeg,png,gif,webp|max:2048', // Cập nhật cho file
        ], [
            ...
            'variants.*.image.file' => 'Hình ảnh phải là một file hợp lệ.',
            'variants.*.image.image' => 'File phải là hình ảnh (JPEG, PNG, GIF, WebP).',
            'variants.*.image.mimes' => 'Hình ảnh chỉ chấp nhận định dạng JPEG, PNG, GIF, hoặc WebP.',
            'variants.*.image.max' => 'Hình ảnh không được lớn hơn :max KB.',
        ]);
        ...
        try {
            ...
            foreach ($validated['variants'] as $index => $variant) {
                $imageFile = $request->file("variants.$index.image"); // lấy file đúng index
                $imagePath = ImageHandler::saveImage($imageFile, 'product_variants');
